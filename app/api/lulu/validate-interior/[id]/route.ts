...
export async function GET(
  ...
  const rate = rateLimitRequest(request, "lulu-validate-interior-status", RATE_LIMIT.limit, RATE_LIMIT.windowMs);
  if (!rate.allowed) {
  ...
  try {
    const response = await luluFetch(`/validate-interior/${validationId}/`, {
      headers: {
    ...
    logInfo("Status de validação de interior consultado", { external_id: externalId, validation_id: validationId });
